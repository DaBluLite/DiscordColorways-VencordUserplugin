...
export function setMany(
    ...
    if (window.BdApi) {
        entries.map(entry => window.BdApi.Data.save("DiscordColorways", entry[0], entry[1]));
...
export function getMany<T = any>(
    ...
    if (window.BdApi) return Promise.all(keys.map(key => window.BdApi.Data.load("DiscordColorways", key)));
    else return customStore("readonly", store =>
...
export function delMany(
    ...
    if (window.BdApi) {
        Promise.all(keys.map(key => window.BdApi.Data.delete("DiscordColorways", key)));
        return new Promise(resolve => resolve(void 0));
    } else return customStore("readwrite", (store: IDBObjectStore) => {
